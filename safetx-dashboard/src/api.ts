...
export function createTrace(payload: TraceRequest): Promise<TraceCreatedResponse> {
  return request<TraceCreatedResponse>("/trace", {
    method: "POST",
    body: JSON.stringify(payload),
  });
}

export function getTrace(jobId: number): Promise<TraceDetail> {
  return request<TraceDetail>(`/trace/${jobId}`);
}
