...
## ✅ Validação contra caso real e sancionado (não só mock)

Conectado o MCP do Blockscout, capturei dado real do pool de 1.000 USDC do
Tornado Cash (`0xd96f2B1c14Db8458374d9Aca76E26c3D18364307`, sancionado pela
OFAC) e rodei o código de produção (`backend/tracing.py`, sem alteração)
contra esses dados via `scripts/validate_against_real_tornado_cash_case.py`.

Resultado: grafo de 2 hops saiu estruturalmente coerente — 7 nós, 12
arestas, deduplicação correta (um endereço com 3 transações reais virou 1
nó só), e o padrão real de lavagem (depósito de 100 USDC → taxa de relayer
~4,4 USDC + saída limpa ~95,6 USDC) apareceu fielmente na proporção das
arestas. O filtro de poeira de stablecoin (`< 1 unidade`) corretamente
excluiu 2 transferências reais sub-$1, validando que o filtro funciona
como desenhado também contra dado real, não só contra caso sintético.

Achado lateral: o endereço de "saída limpa" redepositou repetidamente em
outro contrato (provável outro mixer) — padrão de "reciclagem" que o
grafo evidencia naturalmente pela repetição do nó de destino.
